...
def _show_image_safely(
    ...
    *,
    ...
    """OneDrive同期中・破損ファイルでも画面全体を止めずに画像を表示する。"""
    try:
        ...
        # 画像の破損・OneDriveの同期中・対応外形式などは、該当画像だけを飛ばす。
        return False
...
def _render_context_images(title: str, paths: list[Path]) -> None:
    ...
    for index, path in enumerate(paths[:3]):
        with columns[index % len(columns)]:
            if _show_image_safely(path, caption=str(path.relative_to(source_dir)), width=180):
                with st.expander("🔍 タップして拡大"):
                    _show_image_safely(path, use_container_width=True)
            else:
                st.caption(f"画像を読み込めませんでした: {path.name}")
...
def find_event_logo_path(event_name: str) -> str | None:
    """公演ロゴを優先し、なければキービジュアルを公演レポート用に返す。"""
    if _PUBLIC_MODE:
        return None
    event_key = _name_key(event_name)
    ...
    for raw_path in _material_image_paths():
        path = Path(raw_path)
        is_logo = bool(re.search(r"公演ロゴ|logo", path.stem, flags=re.IGNORECASE))
        is_key_visual = "キービジュ" in path.stem
        if not is_logo and not is_key_visual:
            continue
        category, hint = _image_link_hint(path)
        hint_key = _name_key(hint)
        if hint_key and (hint_key in event_key or event_key in hint_key):
            if is_logo:
                return str(path)
...
def find_event_setlist_image_paths(event_names: list[str]) -> list[str]:
    """指定したDAYに対応する、ローカルの公式セットリスト画像を順番に返す。"""
    if _PUBLIC_MODE:
        return []
    ...
    for event_name in event_names:
        ...
        for raw_path in _material_image_paths():
            path = Path(raw_path)
            if "セットリスト" not in path.stem:
                continue
            category, hint = _image_link_hint(path)
            hint_key = _name_key(hint)
            if category != "公演" or not hint_key or not (hint_key in event_key or event_key in hint_key):
                continue
            if day_match and f"DAY{day_match.group(1)}" not in path.stem.upper():
                continue
            path_text = str(path)
