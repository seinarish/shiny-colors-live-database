    ...
    if PUBLIC_MODE:
        ...
        with tab18:
            render_page_header(
                "ℹ️",
                "このサイトについて",
                "公開範囲・データの見方・権利に関する案内です。",
            )
            st.subheader("このサイトの目的")
            st.write("シャイニーカラーズのライブに関する記録を、比較・検索しやすい形でまとめたファンサイトです。")

            st.subheader("掲載しているデータ")
            st.markdown(
                """
                - 登録済みの公演・セットリストをもとにした披露記録
                - シャイニーカラーズのキャストまたはアイドルが歌唱した記録
                - 合同ライブなどで、他ブランドのキャスト・アイドルだけがシャイニーカラーズ楽曲をカバーした記録は集計対象に含めていません。
                - 公演・楽曲の分類は、このサイトで比較しやすくするための独自分類です。
                """
            )

            st.subheader("権利・クレジット")
            st.markdown(
                """
            ...
            st.markdown(
                """
                - 表記ゆれはできるだけ統一していますが、内容に誤りが含まれる可能性があります。参考情報としてご利用ください。
                - 公式発表や各公式ページの情報を優先してください。
                """
            )
