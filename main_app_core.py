...
- **キャスト名とアイドル名を同一視**：キャスト名を選んだとき、そのキャストがアイドルとして歌唱した記録も一緒に表示・集計します（例：XRライブなどでキャラクター名義で歌唱した記録）。
- **ショート版を同一曲として合算**：short版やVer.違いを、元の楽曲とまとめて数えます。ただし「Migratory Echoes(short)」は、選択していても別の楽曲として扱います。
        ...
        with tab18:
            ...
            st.subheader("権利について")
            st.write(
                "このサイトはファンが個人で作っている非公式サイトです。株式会社バンダイナムコエンターテインメントおよび関係各社とは関係ありません。作品名や登場人物名などに関する権利は、それぞれの権利者に帰属します。"
            )
