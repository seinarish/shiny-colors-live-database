...
def render_schedule_prediction() -> None:
    ...
    if selected_test_row is not None:
        is_tour = _is_tour_event(selected_test_row.get(_EVENT_NAME, ""))
        recorded_counts = _tour_show_counts(history)
        tour_show_count = int(recorded_counts.get(selected_test_row.name, 0)) if is_tour else None
        if is_tour:
            st.caption(f"ツアーとして自動判定：記録上の公演数 {tour_show_count}件")
    else:
        tour_kind = st.radio(
            "公演の形式",
            ["通常公演", "ツアー"],
